...
> 版本 0.1.3.14 — 模块化 AI Agent 系统框架，用于学习和研究 Agent 核心架构与设计模式。
...
- 基于 Textual 的终端图形化交互界面：
  ...
  - **Copilot CLI 风格现代极简布局**：
    - **顶部导航栏 (`TopTabBar`)** — 水平排列 `Current`、`Sessions`、`Skills` 紧凑标签，支持鼠标直达、快捷键（`F1`–`F3`）以及 `Tab` / `Shift+Tab` 即时循环切换上一/下一个面板。
    - **极简欢迎横幅 (`WelcomeBanner`)** — 空会话呈现居中 ASCII Mascot 图标、免责声明与动态轮播的 Tip 指引卡片，并在 `/clear` 后优雅恢复。
    - **内联快捷键面板 (`ShortcutHelp`)** — 输入框正上方的紧凑按键速查面板，空输入时按 `?` 即时切换显示/隐藏；`HelpScreen` 模态保留 `/help` 入口。
  ...
  - **现代全屏交互视窗**：
    - **会话管理视窗 (`SessionSelectScreen` / `/sessions`)** — 全屏极简设计，支持实时关键词过滤搜索、全宽亮蓝高光选框、`↑`/`↓` 键盘导航、`e`/`r` 重命名、`Ctrl+X` 再按 `X` 两段式确认删除、`Enter` 恢复会话；右侧预览面板展示会话转录并支持按预览内容过滤。
